...
public class LevelAddCommand extends LevelSubCommand
{
    
    public LevelAddCommand(String permission, SkillLevels plugin)
    {
    ...
    protected void execute(CommandSender sender, Command cmd, String label, String[] args)
    {
        ...
        else
        {
            ...
            if (args[3].equalsIgnoreCase("xp"))
                ls.addExp(Integer.parseInt(args[4]), args[1]);
            else if (args[3].equalsIgnoreCase("points"))
                ls.revokeUsedPoints(Integer.parseInt(args[4]), args[1]);
            else if (args[3].equalsIgnoreCase("level"))
                ls.addLevel(Integer.parseInt(args[4]), args[1]);
        }
    }
